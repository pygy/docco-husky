(function() {
  var check_config, cloc, content_template, destination, docco_styles, docco_template, dox, dox_template, ensure_directory, exec, ext, file_exists, filter_ignored, fs, generate_content, generate_documentation, generate_readme, generate_source_html, get_language, gravatar, highlight, highlight_end, highlight_start, jade, l, languages, parse, parse_args, parse_markdown, path, relative_base, showdown, spawn, walk, write_file, _, _ref;
  ...
  for (ext in languages) {
    ...
    if (l.multi_end === "*/") {
      ...
      l.multi_end_matcher = l.multi_end ? new RegExp(l.multi_end) : new RegExp(/a^/);
    }
  }
  ...
  parse_args = function(callback) {
    ...
    roots = (function() {
      ...
      for (_i = 0, _len = args.length; _i < _len; _i++) {
        a = args[_i];
        _results.push((console.log(a)) || a.replace(/\/+$/, ''));
      }
      return _results;
    })();
    console.log(roots);
  ...
  check_config = function(context, pkg) {
    ...
    return console.log(context.sources.length);
  };

  filter_ignored = function(context) {
    var ignored;
    ignored = new RegExp((context.config.ignore.map(function(pth) {
      return "^\\./" + path.normalize(pth.replace(/\/+$/, ''));
    })).join("|"));
    console.log(ignored);
    return context.sources.filter(function(p) {
      return !(p.match(ignored));
    });
  };
  ...
  parse_args(function(sources, project_name, raw_paths) {
    ...
    return ensure_directory(context.config.output_dir, function() {
      ...
      files = context.sources.slice(0, sources.length + 1 || 9e9);
      next_file = function() {
        if (files.length) {
